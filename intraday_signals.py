...
if stocks_input:
    ...
    for ticker in tickers:
        ...
        # --- Suggested Buy/Sell/Stoploss/Target ---
        ltp = last['LTP']
        atr = last['ATR']

        if signals['Combined Signal'] == "BUY":
            buy_price = round(ltp * 1.002, 2)       # Entry 0.2% above LTP
            stop_loss = round(ltp - atr, 2)         # 1× ATR below
            target = round(buy_price + (2 * atr), 2)  # 2× ATR target
            sell_price = None
        elif signals['Combined Signal'] == "SELL":
            sell_price = round(ltp * 0.998, 2)      # Entry 0.2% below LTP
            stop_loss = round(ltp + atr, 2)         # 1× ATR above
            target = round(sell_price - (2 * atr), 2)  # 2× ATR target
            buy_price = None
        else:
            buy_price = sell_price = stop_loss = target = None

        signals['Buy Price'] = buy_price
        signals['Sell Price'] = sell_price
        signals['Stop Loss'] = stop_loss
        signals['Take Profit'] = target
    ...
    st.dataframe(
        df_signals[['Stock', 'Combined Signal', 'Buy Price', 'Sell Price', 'Stop Loss', 'Take Profit']],
        use_container_width=True
    )
